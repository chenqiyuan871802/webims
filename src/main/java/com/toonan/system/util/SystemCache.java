package com.toonan.system.util;
...
import com.toonan.system.service.SystemCacheService;
...
public class SystemCache {
     ...
     * 创建时间： 2021年9月22日 下午2:58:22 
     * @param 说明
     * @return 说明
     */
	public static List<Menu> getMenuList(UserToken userToken){
		
		List<Menu> menuList=Lists.newArrayList();
		String whetherSuper=WebplusCons.WHETHER_NO;
		if(SystemCons.SUPER_ADMIN.equals(userToken.getAccount())){
			whetherSuper=WebplusCons.WHETHER_YES;
		}
		if(WebplusCons.WHETHER_YES.equals(whetherSuper)){  //如果是超级管理员具有所有的菜单权限
			menuList =SystemCache. getCacheMenu(); 
		}else{
		 menuList = SystemCache.getCacheRoleMenu(userToken.getUserId());
		}
		List<Menu> menuListNew=Lists.newArrayList();
