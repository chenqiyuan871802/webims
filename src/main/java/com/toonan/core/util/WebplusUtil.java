...
public class WebplusUtil {
	...
	public static boolean checkMobile(String mobile) {
		...
		return Pattern.matches("^(13[0-9]|14[5-9]|15[012356789]|166|17[0-8]|18[0-9]|19[0-9])[0-9]{8}$", mobile);
	}
	...
	public static Date stringToDate(String strDate, String srcDateFormat, String dstDateFormat) {
		...
		try {
			if (strDate.indexOf("CST") > -1) {
				tmpDate = new SimpleDateFormat(srcDateFormat, Locale.US).parse(strDate);
			} else {
				tmpDate = (new SimpleDateFormat(srcDateFormat)).parse(strDate, new ParsePosition(0));
			}
	...
	public static String createFileId(){
		char[] charSequence =WebplusCons.UPPER_LETTER .toString().toCharArray();
		Random random = new Random();
		int index = random.nextInt(charSequence.length);
		String randomNum=charSequence[index]+"";
		String fileId=randomNum+WebplusUtil.getCurrentDate("yyMMddHHmmssSSS");
	 ...
	 * @return 说明
	 */
	public static List<String> getListByKey(List<?> objList,String key){
		List<String> strList=Lists.newArrayList();
		if(WebplusUtil.isNotAnyEmpty(objList,key)){
	 ...
	 * @return 说明
	 */
	public static List<Dto> getListDtoByKey(List<?> objList,String keys){
		List<Dto> dataList=Lists.newArrayList();
		if(WebplusUtil.isNotAnyEmpty(objList,keys)){
			for(Object obj:objList){
				Dto dataDto=getDtoByKey(obj,keys);
				dataList.add(dataDto);
	 ...
	 * 创建时间： 2020年3月22日 上午12:32:03 
	 * @param 说明
	 * @return 说明
	 */
	public static Dto getDtoByKey(Object obj,String keys) {
		Dto rtnDto=Dtos.newDto();
		if(WebplusUtil.isNotAnyEmpty(obj,keys)) {
			List<String> keyList=WebplusFormater.separatStringToList(keys);
			Dto dataDto=Dtos.newDto();
			if(obj instanceof Map) {
				dataDto.putAll((Map)obj);
			}else {
			...
			for(String key:keyList) {
				Object value=dataDto.get(key);
				rtnDto.put(key, value);
			}
		}
		return rtnDto;
	}
	...
	/**
	 * 截取字符串最后的数字 简要说明： 编写者：陈骑元 创建时间：2017年9月14日 上午11:31:59
	 *
	 * @param 说明
	 * @return 说明
	 */
	public static String subSringEndNum(String str) {
		String rtnStr = "";
		String regex = "\\d*";
	...
	/**
	 * 
	 * 简要说明：获取字符串第一个中文索引的位置
	 * 编写者：陈骑元（chenqiyuan@toonan.com）
	 * 创建时间： 2020年4月10日 下午5:13:00 
	 * @param 说明
	 * @return 说明
	 */
	 ...
	 * @return 说明
	 */
	public static List<String> removeStrByList(String str,List<String> strList) {
    ...
    public static String toStringNotNull(Object obj) {
        if (isEmpty(obj)) {
            ...
            return DateFormatUtils.format((Date) obj, WebplusCons.DATETIME);
        } else if (obj instanceof Collection) {
            Collection coll = (Collection) obj;
            Object collect = coll.parallelStream().map(e -> toStringNotNull(e)).collect(Collectors.joining(WebplusCons.MARK_CSV));
