...
public class R extends HashDto {
	...
	public R() {
		...
		put(WebplusCons.APPMSG_KEY, "操作成功");
	}
    /**
     * 
     * 简要说明：操作错误代码
	...
	public static R error() {
		return error(WebplusCons.ERROR, "操作失败");
	}
    /**
     * 
     * 简要说明：传入错误信息操作代码
	 ...
	 * @return 说明
	 */
	public static R toPage(List<?> dataList,int count) {
		R r=new R();
		r.remove(WebplusCons.APPCODE_KEY);
		r.remove(WebplusCons.APPMSG_KEY);
		r.put("code", 0);
	 ...
	 * @return 说明
	 */
	public static R toLsPage(PageDto pageDto) {
		R r=new R();
		r.remove(WebplusCons.APPCODE_KEY);
		r.remove(WebplusCons.APPMSG_KEY);
		r.put("code", 0);
		r.put("count", pageDto.getCount());
		r.put("data", pageDto.getData());
		return r;
	}
}
