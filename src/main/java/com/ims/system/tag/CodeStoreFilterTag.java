...
public class CodeStoreFilterTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		List<Dict> codeList=CacheCxt.cacheDict(field);
		List<Dict> newcodeList=Lists.newArrayList();
		for(Dict dict:codeList){
			if(!exists(filterArray,dict.getDictCode())){
				newcodeList.add(dict);
			}
		}
		 if(IMSUtil.isNotEmpty(rename)){
			 field=rename;
		 }
		 vmDto.put("field", field);
		 vmDto.put("codeList", newcodeList);
		 StringWriter writer = VelocityHelper.mergeFileTemplate(TagResource.CODE_STORE_VM, vmDto);
