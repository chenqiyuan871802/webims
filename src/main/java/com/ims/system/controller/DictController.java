...
public class DictController extends BaseController {
	...
	public PageDto listDictIndex() {
	    ...
	    SqlHelpUtil.eq(wrapper, "dict_type", pDto,"dictType");
		if(IMSUtil.isNotEmpty(queryParam)){
			wrapper.orNew("dict_key like {0}", "%"+queryParam+"%").or("dict_name like {0}", "%"+queryParam+"%");
		}
	...
	public R saveDict(Dict dict) {
		...
		if (result) {
			CacheCxt.cacheDict(dict.getDictKey()); //刷新字典
			return R.ok();
		} else {
			return R.error("保存失败");
		}
	...
	public R updateDictIndex(DictIndex dictIndex,String oldDictKey) {
		...
		if (result) {
			CacheCxt.cacheDict(dictIndex.getDictKey());
			return R.ok();
		} else {
			return R.error("更新失败");
		}
	...
	public R updateDict(Dict dict,String oldDictCode) {
		if(!oldDictCode.equals(dict.getDictCode())){  //新的字典对照码不等于旧的对照码，则校验重复
			EntityWrapper<Dict> countWrapper = new EntityWrapper<Dict>();
			SqlHelpUtil.eq(countWrapper, "dict_index_id", dict.getDictIndexId());
			SqlHelpUtil.eq(countWrapper, "dict_code", dict.getDictCode());
			int count=dictService.selectCount(countWrapper);
	...
	public R removeDictIndex(String id) {
		...
		if(count>0){
			...
			return R.warn("当前字典对照码下存在只读的字典对照数据，只读的数据不允许修改和删除。");
		}
		...
		if (result) {
			CacheCxt.removeCacheDict(dictIndex.getDictKey());//移除字典对照
			return R.ok();
		} else {
			return R.error("删除失败");
		}
	...
	public R batchRemoveDict(String ids) {
		...
		if (result) {
			Dict dict=dictService.selectById(dictIndexId);
			CacheCxt.cacheDict(dict.getDictKey()); //重新缓存字典数据
			return R.ok();
		} else {
			return R.error("删除失败");
		}
