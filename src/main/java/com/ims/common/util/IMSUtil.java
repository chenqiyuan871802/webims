...
public class IMSUtil {
	...
	public static Date stringToDate(String strDate) {
		Date tmpDate = (new SimpleDateFormat(IMSCons.DATETIME)).parse(strDate, new ParsePosition(0));
		if (tmpDate == null) {
			tmpDate = (new SimpleDateFormat(IMSCons.DATE)).parse(strDate, new ParsePosition(0));
		}
		return tmpDate;
	}
	...
	public static Timestamp getDateTime(int offset) {
		...
		try {
			calendar = new GregorianCalendar();
			formatter = new SimpleDateFormat(IMSCons.DATETIME);
			calendar.set(java.util.Calendar.DAY_OF_MONTH, calendar.get(java.util.Calendar.DAY_OF_MONTH) + offset);
			dateString = formatter.format(calendar.getTime());
			Object obj = TypeConvertUtil.convert(dateString, "Timestamp", IMSCons.DATETIME);
			if (obj != null)
				return (Timestamp) obj;
			else
				return null;
	...
	public static int minutesBetween(Date startDate, Date endDate) {
		try {
			SimpleDateFormat sdf = new SimpleDateFormat(IMSCons.DATETIMEMIN);
			startDate = sdf.parse(sdf.format(startDate));
			endDate = sdf.parse(sdf.format(endDate));
			Calendar cal = Calendar.getInstance();
			cal.setTime(startDate);
	...
	public static String plusDay(int num, String newDate) {
		...
		return plusDay(num, newDate, IMSCons.DATE);
	}

	/**
	 * 
	 * 简要说明：生成随机字符串 编写者：陈骑元 创建时间：2017年2月23日 下午2:08:36
	 * 
	 * @param 说明
	...
	public static String createRandomCode(int randomLength, String randomType) {
		...
		if (IMSCons.RANDOM_TYPE.FULL.equals(randomType)) {
			randomSb.append(IMSCons.UPPER_LETTER).append(IMSCons.LOWCASE_LETTER).append(IMSCons.NUMBER_LETTER);
		} else if (IMSCons.RANDOM_TYPE.UPPER.equals(randomType)) {
			randomSb.append(IMSCons.UPPER_LETTER);
		} else if (IMSCons.RANDOM_TYPE.LOWCASE.equals(randomType)) {
			randomSb.append(IMSCons.LOWCASE_LETTER);
		} else if (IMSCons.RANDOM_TYPE.NUMBER.equals(randomType)) {
			randomSb.append(IMSCons.NUMBER_LETTER);
		} else if (IMSCons.RANDOM_TYPE.UPPER_NUMBER.equals(randomType)) {
			randomSb.append(IMSCons.UPPER_LETTER).append(IMSCons.NUMBER_LETTER);
		} else if (IMSCons.RANDOM_TYPE.UPPER_LOWCASE.equals(randomType)) {
			randomSb.append(IMSCons.UPPER_LETTER).append(IMSCons.LOWCASE_LETTER);
		} else {
			randomSb.append(IMSCons.LOWCASE_LETTER).append(IMSCons.NUMBER_LETTER);
		}
	...
	public static String getIeVersion(String userAgent) {
		String ieVersion = "";
		if (isIE(userAgent)) {
			userAgent = IMSUtil.trimAll(userAgent);
	...
	/**
	 * 使用不可见字符串IMSCons.MARK_CSV将字符串隔开，重新组成一串字符串
	 * 
	 * @param strs
	 * @return
	 */
	public static final String joinStringWithUnseeChar(Collection<String> strs) {
		return joinStringWithSplitor(strs, IMSCons.MARK_CSV);
	}
	 ...
	 * @return返回一个格式化的时常
	 */
	public static String changeMinuteFormat(int minuteNum) {

		String tempStr = "";
		int day=minuteNum/(60*24);
		int hour=(minuteNum-60*24*day)/60;
		int minute=minuteNum-60*24*day-hour*60	;
		tempStr += day + "天";
		tempStr += hour + "小时";
		tempStr += minute + "分";
	 ...
	 * 简要说明：
	 * 编写者：陈骑元
	 * 创建时间：2017年6月27日 下午9:59:20
	 * @param 说明
	 * @return 说明
	 */
    public static String  addTime(String startTime,int addMinute){
    	  String[] array=startTime.split(":");
    	  String hourStr=array[0];
    	  String minuteStr=array[1];
    	  int hour=Integer.parseInt(hourStr);
    	  int minute=Integer.parseInt(minuteStr);
    	  int totalMinute=minute+addMinute;
    	  int hourNum=totalMinute/60;
    	  hour+=hourNum;
    	  int remainMintue=totalMinute-60*hourNum;
	...
	public static void main(String args[]) throws ParseException {
		...
		System.out.println(addTime("9:00",120));
	}
}