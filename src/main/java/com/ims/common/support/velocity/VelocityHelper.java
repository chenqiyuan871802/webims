package com.ims.common.support.velocity;
...
import com.ims.common.exception.WebimsException;
...
public class VelocityHelper {
	...
	static{
		...
		try {
			...
			throw new WebimsException("初始化Velocity模板引擎实例失败", e);
		}
	}
	...
	private static Properties getDefaultProperties() {
		...
		try {
			...
			throw new WebimsException("读取Velocity模板引擎属性配置文件出错[velocity.ims.properties]", e);
		}
		return props;
	}
	...
	public static StringWriter mergeStringTemplate(String pTemplateString, Dto pDto) {
		if (IMSUtil.isEmpty(pTemplateString)) {
		...
		try {
			...
			throw new WebimsException("字符串模板合并失败", e);
		}
		return writer;
	}
	...
	public static StringWriter mergeFileTemplate(String pTemplatePath, Dto pDto) {
		if (IMSUtil.isEmpty(pTemplatePath)) {
		...
		try {
			...
			throw new WebimsException("解析文件模板失败", e);
		}
		VelocityContext context = VelocityHelper.convertDto2VelocityContext(pDto);
		try {
			template.merge(context, writer);
		} catch (Exception e) {
			throw new WebimsException("文件模板合并失败", e);
		}
		return writer;
	}
